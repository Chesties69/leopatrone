...
export default function GalleryScrollProjector({ images }) {
  const [imageDimensions, setImageDimensions] = useState(null);
  const [maxAspectRatio, setMaxAspectRatio] = useState(null);
  const [minAspectRatio, setMinAspectRatio] = useState(null);
  ...
  useEffect(() => {
    ...
    Promise.all(promises).then((values) => {
      setImageDimensions(values);
      setMaxAspectRatio(maxRatio);
      setMinAspectRatio(minRatio);
    });
  }, [images]);
